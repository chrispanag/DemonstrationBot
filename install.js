...
try {
  ...
  const promise = fetch(`https://graph.facebook.com/v2.11/me/messenger_profile?${qs}`, {
    method: 'POST',
...
let promise = fetch(`https://graph.facebook.com/v2.6/${FB_PAGE_ID}/subscribed_apps?${qs}`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
})
.then(rsp => rsp.json()).then(json => {
...
if (NLP_ENABLED) {
  ...
  fetch(`https://graph.facebook.com/v2.11/me/nlp_configs?nlp_enabled=${NLP_ENABLED}&custom_token=${WIT_TOKEN}&${qs}`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
  })
  .then(rsp => rsp.json()).then(json => {
